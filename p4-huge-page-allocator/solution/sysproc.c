...
int sys_hugesbrk(void)
{
  ...
  if (argint(0, &n) < 0)
    return -1;
  if(argint(1, &flag) < 0)
    return -1;  // Flag must be provided

  // check if flag is passed
  // if huge page flag is set, adjust n to mark as a huge page request
  if (flag == VMALLOC_SIZE_HUGE)
  {
    addr = myproc()->hugesz + HUGE_VA_OFFSET;
    n += HUGE_VA_OFFSET; // signaling to growproc that its a huge page alloc
  }
  else if (flag == VMALLOC_SIZE_BASE)
  {
    addr = myproc()->sz;
  }
  else
